...
public class MetroDAO {
	...
	public boolean fermateCollegate(Fermata f1, Fermata f2) {
		...
		try {
			...
			conn.close(); //implica rs.close() e st.close() 
	...
	public List<Connessione> getAllConnessioni(List<Fermata> fermate) {
		String sql = "SELECT  id_connessione, id_linea, id_stazP, id_stazA "
				+ "FROM connessione "
				+ "WHERE id_stazP>id_stazA" ; // per evitare WHERE (id_stazP=? AND id_stazA=?) OR (id_stazP=? AND id_stazA=?) 
		...
		try {
			...
			while(rs.next()) {  
				...
				//in questo caso con il risultato della query mi creo all'interno del metodo direttamente gli oggetti
